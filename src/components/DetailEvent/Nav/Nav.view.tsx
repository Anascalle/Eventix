...
import InvitationsAccept from "../../Main/AceptedInvitations/AceptedInvitations";
const Nav3: React.FC = () => {
  ...
  // Listener de las invitaciones aceptadas
  useEffect(() => {
    if (userId) {
      ...
      const unsubscribeNotifications = onSnapshot(notificationsRef, (docSnap) => {
        if (docSnap.exists()) {
          ...
          // Verificar si la invitación de aceptar está activa
  ...
  const toggleModal = () => {
    ...
    setHasNotification(false); // Oculta el círculo de notificaciones al abrir el modal
  };

  return (
    <nav className="nav_bar3">
